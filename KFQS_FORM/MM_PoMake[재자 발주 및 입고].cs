    ...
    public partial class MM_PoMake : DC00_WinForm.BaseMDIChildForm
    {
        ...
        private void MM_PoMake_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                dtTemp = _Common.Standard_CODE("UNITCODE");                                                //UNITCODE 기준정보 가져와서 데이터 테이블에 추가.
                UltraGridUtil.SetComboUltraGrid(this.grid1, "UNITCODE", dtTemp, "CODE_ID", "CODE_NAME");   //셀을 콤보박스로 지정해서 사용할수 있음 


                dtTemp = _Common.GET_TB_CUSTMATTER_CODE("");                 //데이터 테이블에 표현할 데이터 가져오기.          
                Common.FillComboboxMaster(this.cboCust_H, dtTemp,            // 조회에 있는 콤보박스 컨트롤에 데이터 등록
                                          dtTemp.Columns["CODE_ID"].ColumnName,
                                          dtTemp.Columns["CODE_NAME"].ColumnName,
                                          "ALL", "");
                UltraGridUtil.SetComboUltraGrid(this.grid1, "CUSTCODE", dtTemp, "CODE_ID", "CODE_NAME");   //그리드에 있는 해당 컬럼에 콤보 박스형태로 데이터 등록
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                DataTable dtTemp = new DataTable();
                dtTemp = helper.FillTable("16MM_PoMake_S1", CommandType.StoredProcedure
        ...
        public override void DoDelete()
        {
            ...
            {
                ShowDialog("입고된 발주 내역은 삭제 할수 없습니다.", DC00_WinForm.DialogForm.DialogType.OK);
                return;
            }
                ...
                foreach(DataRow drrow in dtTemp.Rows)
                {
                    switch (drrow.RowState)
                    {
                        case DataRowState.Deleted:    //삭제
                            drrow.RejectChanges();
                            helper.ExecuteNoneQuery("16MM_PoMake_D1", CommandType.StoredProcedure,
                        ...
                        case DataRowState.Added:     //추가
                            ...
                            helper.ExecuteNoneQuery("16MM_PoMake_I1"
                        ...
                        case DataRowState.Modified:  //저장
                            helper.ExecuteNoneQuery("16MM_PoMake_U1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("PLANTCODE", Convert.ToString(drrow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
